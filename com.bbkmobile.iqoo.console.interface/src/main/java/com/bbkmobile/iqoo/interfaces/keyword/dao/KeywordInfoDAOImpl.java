...
public class KeywordInfoDAOImpl extends AnnotationBaseDao implements
        KeywordInfoDAO {

    // private char recommend = '1';

    public List<CensorWord> getHitCensorWoreList(String content)
            throws Exception {
        String sql = " select word,grade from t_censor_word where ? regexp word order by grade";
        @SuppressWarnings("unchecked")
        List<CensorWord> censorWords = getSession()
                .createSQLQuery(sql)
                .addScalar("word", Hibernate.STRING)
                .addScalar("grade", Hibernate.INTEGER)
                .setString(0, content)
                .setResultTransformer(
                        Transformers.aliasToBean(CensorWord.class)).list();
    ...
    // }
    // }
    // public List<CensorWord> getCensorWords(final CensorWord censorWord)
    // throws Exception{
    // try {
    // @SuppressWarnings("unchecked")
    // List<CensorWord> list = this.getHibernateTemplate().executeFind(new
    // HibernateCallback() {
    ...
    // }
    //
    // if(null!=censorWord.getType() && !censorWord.getType().equals("") &&
    // 0!=censorWord.getType()){
    // criteria.add(Restrictions.eq("type", censorWord.getType()));
    // }
    //
    // if(null!=censorWord.getGrade() && !censorWord.getGrade().equals("") &&
    // 0!=censorWord.getGrade()){
    ...
    // }
    //
    // public boolean deleteCensorWords(CensorWord censorWord, String ids)
    // throws Exception {
    ...
    // criteria.add(Restrictions.between("add_date", keyword.getFrom_date(),
    // keyword.getTo_date()));
    // }
    ...
    // @SuppressWarnings("unchecked")
    // @Override
    // public List<Keyword> findKeywordByProperty(final int start, final Keyword
    ...
    //
    // @SuppressWarnings({ "unchecked", "rawtypes" })
    // public int countAllRecommendKeyword(final Keyword keyword) throws
    // Exception {
    // int result = 0;
    // List<Keyword> list = this.getHibernateTemplate().executeFind(
    // new HibernateCallback() {
    ...
    //
    // @SuppressWarnings("unchecked")
    // public List<Keyword> getRecommendKeywordInfo(final int start, final
    ...
    // try{
    // return
    // (Keyword)getHibernateTemplate().find("from Keyword where id=?",id).get(0);
    // }catch(Exception e){
    // throw e;
    // }
    // }
    ...
    //
    // @Override
    // public boolean updateSequenceAndRank(Long id, int sequence, int rank)
    // throws Exception {
    // try{
    // String queryString
    // ="update Keyword set sequence=?,rate_rank=? where id=?";
    // Session session=getSession();
    // Query queryObj=session.createQuery(queryString);
    ...
    //
    // @Override
    // public boolean updateRecommendKeyword(Long id, Character recommend)
    // throws Exception {
    // try{
    ...
    // public List<KeywordClick> findClickForKeyword(final Keyword keyword,Date
    // from_date,Date to_date) throws Exception {
    // try {
    // Query queryObj ;
    ...
    // if (null != from_date){
    // String queryString =
    // "from KeywordClick where keyword=? and click_date>=? and click_date<=?";
    // Session session = getSession();
    // queryObj = session.createQuery(queryString);
    // queryObj.setEntity(0, keyword);
    ...
    // @SuppressWarnings("unchecked")
    // //参数from_date和to_date为保留参数，方便统计某时间段点击次数
    // public List<KeywordSearch> findSearchForKeyword(Keyword keyword,Date
    ...
    // if (null != from_date){
    // String queryString =
    // "from KeywordSearch where keyword=? and search_date>=? and search_date<=?";
    // Session session = getSession();
    // queryObj = session.createQuery(queryString);
    // queryObj.setEntity(0, keyword);
    // queryObj.setTimestamp(1, from_date);
