    ...
    public String comments() throws Exception {
        ...
        try {
            ...
            String page_index = StringUtils.defaultIfEmpty(
                    getHttpServletRequest().getParameter("page_index"), "1");
            ...
            if (null != app_id && !"".equals(app_id.trim())
                    && StringUtils.isNumeric(app_id)) {
                requestParameter.setIdlong(Long.parseLong(app_id));
            }
            if (null != apps_per_page && !"".equals(apps_per_page.trim())
                    && StringUtils.isNumeric(apps_per_page)) {
                requestParameter.setApps_per_page(Integer
                        .parseInt(apps_per_page));
            }
            if (null != page_index && !"".equals(page_index.trim())
                    && StringUtils.isNumeric(page_index)) {
            ...
            String data = commentReviewService
                    .getAppCommentListForXml(requestParameter);
            if (appVersion != null && StringUtils.isNumeric(appVersion)
            ...
            Lg.error(LgType.COMMENT_REVIEW, "手机获取app用户评论信息时出错，id=" + app_id
                    + ",target=" + target + ",cs=" + cs + ",appVersion="
