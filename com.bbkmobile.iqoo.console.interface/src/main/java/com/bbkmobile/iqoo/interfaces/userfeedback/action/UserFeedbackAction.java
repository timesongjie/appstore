...
public class UserFeedbackAction extends BaseAction {
    ...
    public String postComments() throws Exception {
        try {
            ...
            Lg.error(LgType.USERFEEDBACK,
                    "保存手机用户评论信息时出错，error=" + e.getMessage());
        }
        return null;
    }
    ...
    public String reporterr() throws Exception {
        try {
            ...
            String client_ip = RequestUtil.getClientIP(request);
            // sucess =
            // commentReviewService.saveComment(app_id,score,content,ip);
            String imei = request.getParameter("imei");
            ...
            String xml_data = errorReportService.saveErrorReport(app_id,
                    user_name, model, content, client_ip, imei, errCode, type,
                    target, elapsedtime, appVersion);
            outwrite(xml_data, "text/xml;charset=utf-8");
        } catch (Exception e) {
            // e.printStackTrace();
    ...
    // String postfix = sf.format( new Date());
    // tableName = tableName.substring(0, tableName.indexOf("yyyyMM")) +
    // postfix;
    // }
    ...
    /*
     * public String reporterr() throws Exception{ try { HttpServletResponse
     * reponse = ServletActionContext.getResponse(); HttpServletRequest request
     * = ServletActionContext.getRequest(); String user_name =
     * request.getParameter("user_name"); String app_id =
     * request.getParameter("id"); String content =
     * request.getParameter("content"); String client_ip =
     * RequestUtil.getClientIP(request); //sucess =
     * commentReviewService.saveComment(app_id,score,content,ip); String imei=
     * request.getParameter("imei"); String errCode =
     * request.getParameter("errCode");
     * 
     * String xml_data = userFeedbackService.saveErrorReport(app_id, user_name,
     * null, content, client_ip, imei, errCode);
     ...
     * out.write(xml_data.getBytes("utf-8")); out.flush(); out.close();
     * 
     * 
