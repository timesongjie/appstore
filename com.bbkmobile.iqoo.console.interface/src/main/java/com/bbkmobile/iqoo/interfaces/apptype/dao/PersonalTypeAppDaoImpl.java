...
public class PersonalTypeAppDaoImpl extends AnnotationBaseDao implements
        PersonalTypeAppDao {

    @Override
    public List<CommonResultAppInfo> listForInterface(final int typeId,
            final PageVO page, String orderBy, final Model model,
                        ...
                        return AppInfoFilter.execute(session, page, model,
                                new SqlBuffer() {
                                    ...
                                    public String orderSq() {
                                        if (order != null) {
                                            if ("3".equals(order)) {
                                                return " order by appinfo1_.downloadCount  desc ";
                                            } else if ("1".equals(order)) {
                                                return " order by appinfo1_.updateDate  desc ";
                                            }
                                        }
    ...
    public int count(PersonalTypeApp app) throws Exception {
        Criteria createCriteria = getSession().createCriteria(
                PersonalTypeApp.class);
        if (app != null) {
            if (app.getType() != null && app.getType().getId() != null) {
                Criteria criterion = createCriteria.createCriteria("type",
                        Criteria.LEFT_JOIN);
        ...
        Integer result = ((Integer) createCriteria.setProjection(
    ...
    public List<PersonalType> list(PersonalType type) throws Exception {
        ...
        return session.createCriteria(PersonalType.class)
