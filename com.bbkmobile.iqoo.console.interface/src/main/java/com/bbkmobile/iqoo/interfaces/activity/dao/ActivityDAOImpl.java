...
public class ActivityDAOImpl extends AnnotationBaseDao implements ActivityDAO {
    ...
    public List<ModelActivity> getModelActivities() throws Exception {
        ...
        Query query = getSession()
                ...
                .setResultTransformer(
                        Transformers.aliasToBean(ModelActivity.class));
    ...
    @Override
    public List getModelActivityIdByAppId(Long app_id) throws Exception {
        String sql = "select act_id,name from t_model_activity where app_id = ? order by show_order ";
        return getSession().createSQLQuery(sql).setLong(0, app_id).list();
    }
