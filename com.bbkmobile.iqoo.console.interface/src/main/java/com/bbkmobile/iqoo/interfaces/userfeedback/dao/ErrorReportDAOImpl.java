...
public class ErrorReportDAOImpl extends AnnotationBaseDao implements
    ...
    public boolean saveAppInstallSucessLog(ErrorReport errorReport,
            String target, String elapsedtime, String appVersion)
            throws Exception {
        try {
            String table_name = "";
            ...
            String sql = "insert into " + table_name
                    + "(imei,model,ip,app_id,elapsedtime,version) " + "value('"
                    + errorReport.getUser_imei() + "','"
                    + errorReport.getModel() + "','" + errorReport.getUser_ip()
                    + "','" + errorReport.getApp_id() + "','" + elapsedtime
                    + "','" + appVersion + "')";
    ...
    public boolean saveErrorReport(AppInfo appInfo, String user_name,
        ...
        if (null != appInfo) {
            ...
            List list = getHibernateTemplate().find(
                    "from ConsoleConstant where type=? and value=?",
                    Short.parseShort(type.toString()), errCode);
    ...
    @Override
    public boolean saveBaiduErrorReport(Long app_id, String user_name,
            String model, String content, String client_ip, String imei,
            String errCode, Character type) throws Exception {
        boolean result = false;
