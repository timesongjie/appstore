...
public class SearchXmlProcessor {
    ...
    public boolean CheckUnicodeString(String value) {
        for (int i = 0; i < value.length(); ++i) {
            if (value.charAt(i) > 0xFFFD) {
                ...
                return false;
            } else if (value.charAt(i) < 0x20 && value.charAt(i) != '\t'
                    && value.charAt(i) != '\n' && value.charAt(i) != '\r') {
    ...
    public SearchResultOneVO parseSearchResultOne(Element root) {
        ...
        for (Element ele : ls) {
            if (SearchConstants.TAGS_RESPONSE_STATUSCODE.equals(ele.getName())) {
            ...
            } else if (SearchConstants.RESPONSE_STATUSMESSAGE.equals(ele
                    .getName())) {
                ...
                if (resultEleLs.size() > 0
                        && SearchConstants.APPS_APP.equals(resultEleLs.get(0)
                                .getName())) {
    ...
    public String downloadurl(String url) {
        ...
        for (retry = 0; retry < 3; retry++) {
            ...
            /*
             * if (HttpStatus.SC_OK == statusCode) { Header locationHeader =
             * method.getResponseHeader("location"); if (locationHeader != null)
             * { location = locationHeader.getValue(); } break; }
             */
            /*
             * if (statusCode == HttpStatus.SC_MOVED_PERMANENTLY || statusCode
             * == HttpStatus.SC_MOVED_TEMPORARILY) { // 从头中取出转向的地址 Header
             * locationHeader = method.getResponseHeader("location"); if
    ...
    public String createXmlOneForPhone(RequestParameter requestParameter,
            String xmlFromBaidu) {
         ...
         * if(version<300){
         * 
         * //if(!url.matches(".*\\.apk")){ String urlRedirect =
         * downloadurl(app.getUrl());
         * //System.out.println("urlRedirect="+urlRedirect);
         * if(null!=urlRedirect){ url=urlRedirect; } // } }
         */
        String baidu_id = "";
        String local_url = "";
        ...
        if (version >= 500) {
            ...
            local_url = UtilTool.getHttpURL("/appinfo/downloadApkFile?id="
                    + baidu_id + "&app_version="
                    + requestParameter.getApp_version());
        }

        String offical = "0";
        /*
         * if(null!=app.getAuthentication() &&
         * "y".equalsIgnoreCase(app.getAuthentication())){ offical ="1"; }
         */
        ...
        if (null == app.getScore()) {
            ...
            packageEle.addElement("score").setText(
                    Integer.parseInt(app.getScore()) / 20f + "");
        }
        ...
        if (null != requestParameter.getContent_complete()
                && requestParameter.getContent_complete().equals("1")) { // 补全详细
            AppendAppInfoContentComplete(app, packageEle);
        }

        Element screenshotListEle = packageEle.addElement("ScreenshotList");
        screenshotListEle.addElement("screenshot").setText(
                app.getBigmaplink1() == null ? "" : app.getBigmaplink1());
        screenshotListEle.addElement("screenshot").setText(
                app.getBigmaplink2() == null ? "" : app.getBigmaplink2());
        ...
        packageEle.addElement("download_count").setText(
    ...
    public void AppendAppInfoContentComplete(AppVO app, Element packageEle) {
        ...
        packageEle.addElement("developer").setText(
                SearchConstants.DEVELOPER_FROM_NETWORK);
        packageEle.addElement("raters_count")
                .setText(app.getScore_count() + "");
        packageEle.addElement("version_name").setText(app.getVersionname());
        packageEle.addElement("version_code")
                .setText(app.getVersioncode() + "");
        packageEle.addElement("download_count")
                .setText(app.getDownload_count());
    }

    public String createXmlForPhone2(SearchResultForm requestParas,
            AppInfoListVO vo, String xmlFromBaidu) throws Exception {
        ...
        if (0 == vo_maxpage) {
            packageListEle.addAttribute("maxpage",
                    ((count + numPerPage - 1) / numPerPage) + "");
        } else {
            packageListEle.addAttribute("maxpage",
                    ((count + numPerPage - 1) / numPerPage) + vo_maxpage - 1
                            + "");
        }
    ...
    private void addBaiduAppXml(SearchResultForm requestParas, AppVO app,
            Element packageListEle) throws Exception {
        Element appEle = null;
        AppInfo appInfo = null;
        requestParas.setAppPackageName(app.getPackagename());
        if (systemPackageService.isSystemPackage(app.getPackagename())) {
            return;
        }
        if (appInfoDAO.packageIsExist(app.getPackagename())) {
            if ((appInfo = appInfoDAO.getAppInfoByPackageName(requestParas)) != null) {// by
                                                                                       // time
                if (null != appInfo
                    ...
                    appEle.addElement("icon_url").setText(
                            UtilTool.getDownloadImageHttpURL(appInfo
                                    .getAppIcon()));
            ...
            // 屏蔽来自某些站点的应用
            if (systemPackageService.isFromBlockedSites(app.getSite())) {
                return;
            }
            String baidu_id = null;
            ...
            appEle.addElement("version_code")
                    .setText(app.getVersioncode() + "");
     ...
     * AppInfoListVO vo,String xmlFromBaidu) throws Exception{
     * 
     * int total_count = vo.getTotal_count(); int apps_per_page =
     * vo.getApps_per_page(); int page_index = total_count/apps_per_page + 1;
     * int vo_maxpage = (int)Math.ceil((float)total_count/apps_per_page);
     * 
     * SearchResultVO searchResult = processSearchXml(xmlFromBaidu);
     * 
     * int count = searchResult.getResult().getDisp_num(); int numPerPage =
     * searchResult.getResult().getRn(); int pageNum =
     * searchResult.getResult().getPn();
     * 
     * Document doc = DocumentHelper.createDocument(); Element packageListEle =
     * doc.addElement("PackageList");
     * 
     * if(requestParas.getVersion()>=300){ //if
     * version>=300,返回页码和app总数重新按百度开始（现在不跑这个流程），else,返回本地数据+百度数据
     * packageListEle.addAttribute("maxpage", ((count + numPerPage
     * -1)/numPerPage) + ""); packageListEle.addAttribute("PageNo",
     * (pageNum/numPerPage + 1) + ""); packageListEle.addAttribute("pageSize",
     * numPerPage + ""); packageListEle.addAttribute("TotalCount", count + "");
     * packageListEle.addAttribute("from", SearchConstants.FROM_BAIDU); } else{
     * if(0==vo_maxpage){ packageListEle.addAttribute("maxpage", ((count +
     * numPerPage - 1) / numPerPage) + ""); }else{
     * packageListEle.addAttribute("maxpage", ((count + numPerPage - 1) /
     * numPerPage) + vo_maxpage - 1 + ""); }
     * packageListEle.addAttribute("PageNo", (pageNum / numPerPage + 1) +
     * page_index - 1 + ""); packageListEle.addAttribute("pageSize", numPerPage
     * + ""); packageListEle.addAttribute("TotalCount", count + total_count +
     * ""); packageListEle.addAttribute("from", SearchConstants.FROM_BAIDU);
     * 
     * } Element packageEle = null; String baidu_id = null; for (AppVO app :
     * searchResult.getResult().getApps()) {
     * 
     * //屏蔽来自mumayi应用市场的app //boolean isNotMumayi = true;
     ...
     * if (!appInfoDAO.packageIsExist(app.getPackagename())){
     * 
     * if(requestParas.getVersion()>=500){ //直接返回百度id baidu_id=app.getDocid();
     * }else{ BaiduAppId baiduAppId = new BaiduAppId();
     * baiduAppId.setBaidu_id(app.getDocid());
     * 
     * BaiduAppId baiduAppIdResult = null;
     * 
     * baiduAppIdResult = appInfoDAO.getLocalBaiduAppId(baiduAppId);
     * 
     * if(null==baiduAppIdResult){ baiduAppIdResult =
     * appInfoDAO.saveBaiduAppId(baiduAppId); } baidu_id =
     * "-"+baiduAppIdResult.getId().toString(); //使id变成负数 } packageEle =
     * packageListEle.addElement("Package");
     ...
     * packageEle.addElement("icon_url").setText(app.getIconhdpi()); //
     * packageEle
     * .addElement("icon_url").setText(UtilTool.getDownloadHttpURL(app.
     * getIcon()));
     * 
     * packageEle.addElement("developer").setText(SearchConstants.
     * DEVELOPER_FROM_NETWORK);
     * packageEle.addElement("score").setText((Integer.parseInt(app.getScore())
     * / 20f) + "");
     ...
     * packageEle.addElement("from").setText(SearchConstants.FROM_BAIDU); String
     * offical = "0";
     * 
     * //if(null!=app.getAuthentication() &&
     * "y".equalsIgnoreCase(app.getAuthentication())){ // offical ="1"; //}
    ...
    /*
     * public String createXmlForPC(SearchResultForm requestParas, AppInfoListVO
     * vo,String xmlFromBaidu) throws Exception{
     * 
     * int total_count = vo.getTotal_count(); int apps_per_page =
     * vo.getApps_per_page(); int page_index = total_count/apps_per_page + 1;
     * int vo_maxpage = (int)Math.ceil((float)total_count/apps_per_page);
     ...
     * if(requestParas.getVersion()>=500){ //直接返回百度id baidu_id=app.getDocid();
     * }else{ BaiduAppId baiduAppId = new BaiduAppId();
     * baiduAppId.setBaidu_id(app.getDocid()); BaiduAppId baiduAppIdResult =
     * null;
     * 
     * baiduAppIdResult = appInfoDAO.getLocalBaiduAppId(baiduAppId);
     * 
     * if(null==baiduAppIdResult){ baiduAppIdResult =
     * appInfoDAO.saveBaiduAppId(baiduAppId); } baidu_id =
     * "-"+baiduAppIdResult.getId().toString(); //使id变成负数 }
     * 
     ...
     * packageEle.addElement("size").setText(app.getPackagesize()/1024 + "");
     * packageEle.addElement("from").setText(SearchConstants.FROM_BAIDU); String
     * offical = "0"; packageEle.addElement("offical").setText(offical); } }
     * 
     * return doc.asXML(); }
     */
    public String createXmlForPhone(SearchResultForm requestParas,
            String xmlFromBaidu) throws Exception {
        SearchResultVO searchResult = processSearchXml(xmlFromBaidu);
        int count = searchResult.getResult().getDisp_num();
        int numPerPage = searchResult.getResult().getRn();
        int pageNum = searchResult.getResult().getPn();
    ...
    public AppVO parseApp(Element ele) {
        ...
        for (Element el : ls) {
            if (SearchConstants.APP_SNAME.equals(el.getName())) {
                ...
                if (null != el.getStringValue()
                        && !el.getStringValue().equals("")) {
    ...
    // update app
    public UpdateResultVO processUpdateXml(String xmlFromUpdate)
            throws Exception {
    ...
    // related recommend
    public UpdateResultVO processRelatedRecXml(String relateRecXml)
            throws Exception {
    ...
    // category
    public UpdateResultVO processCategoryXml(String categoryXml)
            throws Exception {
