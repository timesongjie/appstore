...
public class RecommendationServiceImpl implements RecommendationService {
    ...
    // begin:手机接口
    public String findCellphoneRecommendAppList(String apps_per_page,
            String page_index, String model, String cs, float appVersion)
            throws Exception {
        ...
        try {
            ...
            List<CommonResultAppInfo> list = recommendationDAO
                    .getCellphoneRecommendAppsWithFilterModel(page, modelClass);
            ...
            if (null != list) {// (null != list ){//
                ...
                if (null != cs && cs.equals("0")) {
                    if (cellphoneRecommendAppsSize == (apps_per_page_int + 1)) {
                        ...
                        // cellphoneRecommendApps.remove(cellphoneRecommendAppsSize
                        // - 1); // 去掉最后一个
                        list.remove(list.size() - 1);
                    }
                    total_count = list.size();// cellphoneRecommendApps.size();
                                              // // 当前页个数
                    ...
                    if (appVersion == 400 && page_index_int > 1) { // 第一页返回total_count为所有app数量
                        ...
                        total_count = recommendationDAO
                                .countAllCellphoneRecommendAppsWithFilterModel(
                                        series_id, model_id, sdkVersion,
                ...
                if (appVersion >= Constants.APPVERSION530) {
                    ...
                    sb.append("\">");
                    // for(CellphoneRecommendApp cellphoneRecommendAppTmp :
                    // cellphoneRecommendApps){
                    // //sb.append(appInfoDAO.getXmlForAppInfo(cellphoneRecommendAppTmp.getAppInfo(),appVersion));
                    // sb.append(AppInfoTXML.getXmlForAppInfo(cellphoneRecommendAppTmp.getAppInfo(),
                    // appVersion));
    ...
    public String getNewApps(RequestParameter requestParameter)
            throws Exception {
        ...
        try {
            ...
            // List<NewApp> newApps =
            // recommendationDAO.getNewApps(requestParameter,modelClass);
            PageVO page = new PageVO();
            page.setCurrentPageNum(page_index_int);
            page.setNumPerPage(apps_per_page_int);
            List<NewAppsResultAppInfo> apps = null;
            if (appVersion >= Constants.APPVERSION530) {
                apps = recommendationDAO.getNewsAppsWithFilterModel(page,
                        modelClass, false);
            } else {
                apps = recommendationDAO.getNewsAppsWithFilterModel(page,
                        modelClass, true);
            ...
            if (null != apps) {
                ...
                if (1 == page_index_int) { // 第一页返回total_count为所有app数量
                    total_count = recommendationDAO.countNewApps(
                ...
                /*
                 * List<NewApp> newApps; if (null!=cs && cs.equals("0")) { if
                 * (newAppsSize == (apps_per_page_int + 1)) { maxpage =
                 * page_index_int + 1; newApps.remove(newAppsSize - 1); //
                 * 去掉最后一个 } total_count = newApps.size(); // 当前页个数
                 * 
                 * } else { if (null != appVersion && appVersion.equals("400")
                 * && page_index_int > 1) { // 第一页返回total_count为所有app数量 if
                 * (newAppsSize == (apps_per_page_int + 1)) { maxpage =
                 * page_index_int + 1; newApps.remove(newAppsSize - 1); //
                 * 去掉最后一个 } total_count = newApps.size(); // 当前页个数 } else {
                 * total_count =
                 * recommendationDAO.countNewApps(requestParameter, modelClass);
                 * maxpage = (int) Math.ceil((float) total_count /
                 * apps_per_page_int); if (newAppsSize == (apps_per_page_int +
                 * 1)) { newApps.remove(newAppsSize - 1); // 去掉最后一个 } }
                 * 
                 * }
                 */
    ...
    public String getEditorRecommendApps(RequestParameter requestParameter)
            throws Exception {
        ...
        try {
            ...
            String model = requestParameter.getModel();
            modelClass = modelInfoService.findModelByMdName(model);
            List<CommonResultAppInfo> installApps = recommendationDAO
                    .getInstallAppsWithFilterModel(modelClass);
            if (null != installApps) {
    ...
    // end：手机接口
    private String getResultStr(List<CommonResultAppInfo> installApps,
            float app_version) throws Exception {
        StringBuilder sb = new StringBuilder();
        if (app_version >= Constants.APPVERSION530) {
            ...
            // sb.append(appInfoDAO.getXmlForAppInfo(editorRecommendAppTmp.getAppInfo(),
            // appVersion));
            sb.append("</PackageList>");
        }
        return sb.toString();
    ...
    private String getResultStr(PageVO vo, List<NewAppsResultAppInfo> apps,
        ...
        if (appVersion >= Constants.APPVERSION530) {
            ...
            for (NewAppsResultAppInfo newAppTmp : apps) {
                sb.append(AppInfoTXML.getXmlForAppInfoWithShotscreen(newAppTmp,
                        appVersion));
            }
            sb.append("</PackageList>");
        }
